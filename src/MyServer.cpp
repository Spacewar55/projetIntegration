...
void MyServer::initAllRoutes() { 
    ...
    this->on("/backgroundColor", HTTP_GET, [](AsyncWebServerRequest *request) {
        std::string repString = "";
        if (ptrToCallBackFunction) repString = (*ptrToCallBackFunction)("askColor");
        String lireCouleur =String(repString.c_str());
        request->send(200, "text/plain", lireCouleur );
    });
